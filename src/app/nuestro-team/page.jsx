  ...
  useEffect(() => {
    const fetchDJs = async () => {
      try {
        ...
        // Primero obtenemos los DJs activos
        const djsList = [];
        ...
        for (const docSnap of querySnapshot.docs) {
          ...
          try {
            // Buscar en la colección "users" el profileUrl con el mismo id
            const userDocRef = doc(db, "users", djData.id);
            const userDocSnap = await getDoc(userDocRef);

            if (userDocSnap.exists()) {
              const userData = userDocSnap.data();
              djData.profileUrl = userData.profileUrl || "/images/djmitad.png"; // Asignamos profileUrl al objeto DJ
            } else {
              djData.profileUrl = "/images/djmitad.png"; // Valor por defecto si no existe el user
            }
          } catch (userError) {
            console.error("Error fetching user profile:", userError);
            djData.profileUrl = "/images/djmitad.png"; // Valor por defecto en caso de error
          }
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        <div className="h-12 w-12">
                          <Image
                            src={dj.profileUrl || "/images/djmitad.png"}
                            alt={`Foto de perfil de ${dj.nombreDJ}`}
                            width={48}
                            height={48}
