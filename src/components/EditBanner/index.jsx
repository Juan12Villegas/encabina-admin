...
export default function EditBanner() {
    ...
    // Verificar si el usuario tiene permiso para cambiar el banner
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            if (userSnap.exists()) {
                ...
                if (userSnap.data().bannerUrl) {
                    setPreviewUrl(userSnap.data().bannerUrl);
                }
            }
            setLoading(false);
        };
    ...
    const handleUpload = async () => {
        ...
        setUploading(true);
        const uploadToast = toast.loading("Subiendo banner...");

        try {
            const user = auth.currentUser;
            const storageRef = ref(storage, `user-banners/${user.uid}/${selectedFile.name}`);
            ...
            if (userData?.bannerUrl) {
                try {
                    const oldImageRef = ref(storage, userData.bannerUrl);
            ...
            setUserData(prev => ({ ...prev, bannerUrl: downloadURL }));
            setPreviewUrl(downloadURL);
            setSelectedFile(null);

            toast.success("¡Banner actualizado correctamente!", { id: uploadToast });
        } catch (error) {
            console.error("Error al actualizar el banner:", error);
            toast.error("Error al actualizar el banner. Por favor, intenta nuevamente.", { id: uploadToast });
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <div className="w-full">
            <div className="mx-auto p-4 bg-white shadow-md rounded-lg">
                <div className="mb-6">
                    <p className="text-gray-600">Sube una imagen para personalizar tu banner</p>
                    {!canChangeBanner() && (
                ...
                <div className="flex flex-wrap items-center justify-start gap-4">
                    ...
                    <div className="flex justify-center h-44 w-full lg:w-auto bg-white rounded-lg shadow overflow-hidden">
                        {previewUrl ? (
                            ...
                            <div className="h-44 w-full px-4 bg-gray-200 flex items-center justify-center">
                                <p className="text-gray-500">No hay banner configurado</p>
                            </div>
                        )}
                    </div>
                  ...
                  ${canChangeBanner() ? "file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" : "file:bg-gray-100 file:text-gray-500 hover:file:bg-gray-200"}`}
                                    disabled={uploading || !canChangeBanner()}
                                />
                            ...
                            <button
                                ...
                                disabled={!selectedFile || uploading || !canChangeBanner()}
                                className={`w-full flex items-center justify-center py-2 px-4 rounded-md text-white ${!selectedFile || uploading || !canChangeBanner()
